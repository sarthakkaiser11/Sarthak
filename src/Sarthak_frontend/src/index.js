import { Sarthak_backend } from "../../declarations/Sarthak_backend";
...
appointForm.addEventListener("submit", async (e) => {
  ...
  try {
    const getId = await Sarthak_backend.addTodo(obj.textarea);
...
window.addEventListener("DOMContentLoaded", async () => {
  try {
    const getAll = await Sarthak_backend.getAllTodo();
...
async function deleteTodo(userId) {
  try {
    ...
    await Sarthak_backend.deleteTodo(userId);
